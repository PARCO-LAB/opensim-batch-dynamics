...
def _read_csv(path: Path) -> tuple[list[str], dict[str, np.ndarray]]:
    ...
    return list(reader.fieldnames), {
        name: np.asarray([float(row.get(name, "nan") or "nan") for row in rows], dtype=float)
...
def evaluate_signals(
    ...
    rows.append({"metric": "cop_planar_error", "value": float(np.sqrt(np.mean(cop_error ** 2))) if cop_error.size else math.nan, "unit": "m"})
    return rows
...
def evaluate_trial(
    ...
    for side, index in (("l", 0), ("r", 1)):
        ...
        com_keys = [f"com_acc_{axis}" for axis in "xyz"]
        if all(key in data for key in com_keys):
            active = np.column_stack([
                np.interp(pred_t, ref_t, gt_grf[:, foot, 2]) > 0.05 * mass_kg * 9.81
                for foot in range(2)
            ])
            total = mass_kg * (np.column_stack([data[key] for key in com_keys]) - np.array([0.0, 0.0, 9.81]))
            active_count = np.maximum(active.sum(axis=1), 1)
            com_baseline = total / active_count[:, None]
...
def main() -> int:
    ...
    if args.mode == "evaluate":
        ...
        for pair in pairs:
            candidates = [
                args.pipeline_dir / pair.subject / f"{pair.motion_path.stem}.csv",
                args.pipeline_dir / f"{pair.motion_path.stem}.csv",
                args.pipeline_dir / f"{pair.trial}.csv",
            ]
